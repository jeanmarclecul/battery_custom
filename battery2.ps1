...
$form.Size = New-Object System.Drawing.Size(400, 50)
...
$timer.Add_Tick({
        ...
        if ($BTHDevices) {
            ...
            if ($BatteryLevels) {
                ...
                $Message = "$($BTDeviceFriendlyName): $BatteryLevels % $rotatingChar"
                Write-Host $rotatingChar
                $label.Text = $Message
            }
            else {
                $label.Text = "No battery level information found for $($BTDeviceFriendlyName) devices."
            }
        }
        else {
            $label.Text = "Bluetooth device not found."
        }
    })
